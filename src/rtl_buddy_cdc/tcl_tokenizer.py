"""Tcl-aware SDC word tokenizer — the fallback reader (rtl-buddy-cdc#298).

This module is deliberately **stdlib-only and dependency-free**: it
imports nothing from :mod:`rtl_buddy_cdc` (no :class:`ClockSpec`, no
logging config, no typer). rtl-buddy vendors this file verbatim at a
pinned commit and diff-checks it in CI (rtl-buddy/rtl_buddy#641), so
the two functions below must stay byte-identical on both sides. Add
nothing here that reaches back into the package.

:func:`_tokenize` splits SDC source into one word list per command
line, respecting the Tcl-flavoured quoting SDC actually uses.
:func:`_extract_names` peels a single word (``{a b}`` /
``[get_ports a]`` / ``a``) into identifier names.

Two-tier design: :mod:`rtl_buddy_cdc.sdc` prefers a ``tkinter.Tcl()``
safe-interp reader, which *evaluates* ``set`` / ``expr`` / command
substitution, and falls back to this tokenizer when ``_tkinter`` is
not importable. The "switch to ``tkinter.Tcl()``" note left in
:func:`_tokenize`'s docstring by issue #144 has been acted on — the
docstring text is kept verbatim so the vendoring diff-check keeps
passing.
"""
