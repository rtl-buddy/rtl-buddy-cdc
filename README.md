...
- A blackboxed instance of a **recognised CDC macro** (`xpm_cdc_*`, or a `--sync-primitive` registration) is exempt from the decline above: it is dual-clock *by design*, and the macro is the synchroniser. It is summarised at its destination clock instead — see [XPM CDC macro recognition](#xpm-cdc-macro-recognition).
