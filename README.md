...
| `--incdir DIR` / `-I DIR` | optional | Directory searched by `` `include `` after the including file's own directory — the filelist `+incdir+` of the design. **Repeatable**, searched in order. Reaches both frontends (`-I` on `read_verilog` / `read_slang`; pyslang's include path under `--frontend slang`). A relative directory resolves against `--project-root`; a directory that does not exist is rejected up front (exit 2). |
