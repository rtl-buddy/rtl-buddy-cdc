...
`--blackbox` buys scale by *discarding* the module's body, and that trade
is what forces the declines above: with no internals there is no way to
check reconvergence or synchroniser depth inside the block, so the
analyzer refuses to abstract the shapes where those matter.

`lint --greybox MODULE` keeps the body. The module still becomes a
boundary cell — the parent is never flattened through it, so the scaling
win is identical — but the analyzer additionally runs the ordinary
pipeline over the module **once per (module type, clock context)**, on its
own internals, and lifts what it finds:

- **A proven input synchroniser stops over-reporting.** A single-bit
  foreign-domain input landing on a real 2FF chain used to fire CDC-001 at
  every abstracted boundary, because a boundary sink has nothing behind it
  to walk. The measured depth is now published, so CDC-001 is silent
  exactly where the flattened design is silent — and `--sync-depth 3`
  still reaches inside and fires CDC-002 on a 2-deep chain.
- **Findings inside the block are reported**, once per module type with
  the instances they cover named, and waivable by boundary instance:

  ```text
  [inside `xsync` — analysed once, 2 instances: u_a, u_b] insufficient
  synchronizer depth on clk_s → clk_c crossing: found 2 flop(s), required >= 3
  ```

- **Internal reconvergence is re-raised at the boundary**, so the
  reconvergence-unsafe decline is retired for greyboxed modules.
- **A multi-clock IP is summarised per port** — each stamped with the
  domain that really captures or launches it — instead of being declined.

The proof bar is deliberately one-sided: `synchronised` is the only lever
in this model that can make the tool *under*-report, so it is set only
when proven. A tie-off, a comb bypass around the chain, a second load on
the input, a multi-bit port, or a chain one stage short all fall back to
the conservative answer.

Three cases still decline, each with its own `CDC-BBX` message: an
internal flop the analyzer cannot place on any of the parent's clock roots
(a clock arriving on an unrecognised pin — a case `--blackbox` used to
abstract away **silently**, so greyboxing strictly *adds* soundness here),
an input captured in two internal domains, and a multi-clock module with
...
|---|---|---|
| **CDC-001** | error | Unsynchronized control crossing — destination flop has no second-stage synchronizer (chain depth = 1). Fires on flop→flop and (when `set_input_delay -clock <c>` types the source) port→flop crossings. Suppressed on `(* cdc_static *)` sources and `(* cdc_handshake *)` capture registers (a single dst register is the intended capture in a req/ack handshake). The depth walk recognises a synchroniser coded as a single **packed shift register** (`reg [N-1:0] s; s <= {s[N-2:0], d}`, tapped from `s[N-1]`) — it lowers to one multi-bit `$dff` whose stages shift intra-cell — and counts its effective depth, so the packed form is accepted identically to the separate-flop form (issue #264). A **synchronous reset** on the same register leaves a `$mux` on the flop's `D` after `proc`; the walk looks through a mux whose other leg is entirely constant (either polarity, any reset value) and measures the depth behind it (issue #301). An *enable* mux — the other leg is the register's own `Q` — is not looked through and still reads as depth 1. The separate-flop form gets the same treatment (issue #304): when a stage's `Q` is read only by the *next* stage's constant-leg synchronous-reset `$mux`, the walk steps over that mux to the next flop, so `if (rst) begin s1 <= '0; s2 <= '0; end else begin s1 <= d; s2 <= s1; end` counts depth 2 on both frontends. |
...
| **CDC-003** | error | Combinational logic between source flop and synchronizer first stage — gate output can glitch and be sampled. A constant-leg synchronous-reset `$mux` sitting directly on the destination flop's `D` is part of the flop, not logic on the crossing path, and is exempt (issue #301) — the slang frontend folds the same source into an `$sdff` and never showed the mux. |
...
| **CDC-014** | error | Combinational logic between synchroniser stages — distinct from CDC-003 (comb feeding stage 1): here a comb cell sits between `stage_1.Q` and `stage_2.D` on the same clock. The gate's propagation delay can sample a metastable head output, destroying the full-cycle resolution time the chain was meant to provide. The chain walker terminates at depth = 1 (the gate breaks the flop→flop continuation), so CDC-001 would mis-report "no second-stage synchronizer" — CDC-001 defers via `_chain_has_inter_stage_comb` when a follow-on flop is present behind a gate, and CDC-014 fires with the correct framing. Suppressed on `(* cdc_sync *)` chain heads and `(* cdc_handshake *)` capture registers (post-capture decode comb is ordinary datapath). The next stage's own constant-leg synchronous-reset `$mux` is not a gate between the stages (issue #304) — the yosys frontend leaves it un-folded after `proc` while slang emits an `$sdff`, and the rule reaches the same verdict on both; an *enable* mux or any mux with a live other leg still fires. |
...
| **CDC-BBX** | error | Blackbox boundary not analysed — emitted by the analyzer's blackbox handling (not the rule pack) when a blackboxed subtree **cannot be soundly abstracted** and is therefore left opaque: it is not provably single-clock (≥2 distinct clock roots, or an unresolved clock), or one of its **output ports drives a clock** (clock generation / forwarding that abstraction would elide), or it is reconvergence-unsafe (≥2 crossings entering distinct input ports, whose internal reconvergence the boundary star-collapse would hide — see [Blackboxing for scale](#blackboxing-for-scale)). With `--greybox` most of these are analysed instead of declined; the ones that remain say which of the three residual reasons applied — see [Compositional boundaries](#compositional-boundaries). The boundary's internal CDC is unanalysed, so this is a **coverage gap**, surfaced per instance and at `error` so it fails by default. Intentional opacity (a separately signed-off IP) is acknowledged by waiving it: `waive CDC-BBX <instance-regex>`. Not produced when a subtree *is* abstracted (single-clock, ≤1 incoming crossing) — those boundary crossings are analysed normally. |
